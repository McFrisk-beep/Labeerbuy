    ...
    return SCModel.extend({
        ...
        update: function update(data) {
            ...
            if (!this.isMultiShippingEnabled || !data.ismultishipto) {
                // If all items of the order are added for pickup, the shipping address should not be set, because the API throws a silent error
                // and the order confirmation email is not sent even though the order is successfully submitted.
                if (
                    !(
                        this.isPickupInStoreEnabled &&
                        data.lines &&
                        _.every(data.lines, function (line) {
                            return _.property('fulfillmentChoice')(line) === 'pickup';
                        })
                    )
                ) {
